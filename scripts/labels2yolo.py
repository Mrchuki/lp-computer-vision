        ...
        for filename in tqdm(filenames):
            ...
            for lp_data in data["lps"]:
                ...
                ocr_output_path = os.path.join(
                    ocr_directory, "images", yolo_split, f"{filename}_{lp_id}.jpg"
                )
                ...
                ocr_img_resized = cv2.resize(
                    ocr_img,
                    (int(ocr_width * rescale_factor_ocr), int(ocr_height * rescale_factor_ocr)),
                )
                ...
                for char_data in lp_data["characters"]:
                    ...
                    ocr_yolo_path = os.path.join(
                        ocr_directory, "labels", yolo_split, f"{filename}_{lp_id}.txt"
                    )
                    append_write_ocr = "a" if os.path.exists(ocr_yolo_path) else "w"
                    with open(ocr_yolo_path, append_write_ocr) as ocr_f:
                        ocr_f.write(
                            create_yolo_bbox_string(class_id, bbox, ocr_width, ocr_height) + "\n"
                        )
